...
import React, { createElement, cloneElement, Children, isValidElement, createContext, Component } from 'react';
import invariant from 'invariant';
import ReactDOM, { render, unmountComponentAtNode } from 'react-dom';
import { getAppContainerProps, renderHtml, hydrateHtml } from 'react-dom-html';
...
function resolveHtml(child, htmlState, options) {
    ...
    if (nestedChildren && Children.count(nestedChildren) > 0) {
        Children.forEach(nestedChildren, function (nestedChild) {
...
function resolveBody(child, htmlState, options) {
    ...
    var childCount = Children.count(nestedChildren);
...
function mapDangerouslySetToState(typeName, typeState, element) {
    ...
    return assignState(typeName, typeState, key, cloneElement(element, { key: key }));
}
...
function mapScriptToState(element, scriptState) {
    ...
    assignState(SCRIPT, scriptState, key, cloneElement(element, { key: key }));
}
...
function mapStyleToState(element, styleState) {
    ...
    return assignState(STYLE, styleState, key, createElement(STYLE, _extends({}, styleProps, {
        key: key,
...
function mapNoScriptToState(element, noScriptState, options) {
    ...
    if (typeof children !== "string") {
        if (!IS_PROD) {
            invariant(isValidElement(children), "Html head <noscript> element only accepts a string or a react component as children.");
        }
        ...
        return assignState(NOSCRIPT, noScriptState, _key, createElement(NOSCRIPT, {
            key: _key
        }, noScriptKids));
    }
    ...
    assignState(NOSCRIPT, noScriptState, key, createElement(NOSCRIPT, _extends({}, noScriptProps, {
        key: key,
...
function mapTitleToState(element, titleState, htmlState) {
    ...
    assignState(TITLE, titleState, TITLE, createElement(TITLE, _extends({}, titleAttributes, {
        key: titleText
    }), titleText));
}
...
function mapElementToState(childType, child, typeState) {
    ...
    return assignState(childType, typeState, key, cloneElement(child, { key: key }));
}
...
var HtmlMetadata = function () {
    ...
    createClass(HtmlMetadata, [{
        ...
        value: function serializeState(renderToString) {
            ...
            return _extends({}, attributes, {
                head: renderToString(createElement(TAG_NAMES.HEAD, headAttributes, createElement(this._headFragment, this.getHeadComponents())))
            });
        }
        ...
        value: function getServerAttributes(element, appContainerOptions) {
            var _getAppContainerProps = getAppContainerProps(element, appContainerOptions),
        ...
        value: function renderAttributes() {
            ...
            ELEMENT_ATTRIBUTE_TUPLES.forEach(function (_ref) {
                ...
                if (typeof tagAttributes !== "undefined") {
                    ...
                    render(createElement(DIV, tagAttributes), tempNode, function () {
    ...
    var tagComponents = tagNames.reduce(function (tags, tagName) {
        ...
        if (tagName === TITLE$1) {
            ...
            if (state[TITLE_TEMPLATE] || state[TITLE$1]) {
                ...
                var titleElement = createElement(TITLE$1, _extends({
...
var _createContext = createContext(HtmlMetadata.createForClientRender().getRootContext()),
...
function Html(props) {
    ...
    var htmlElement = React.createElement(HTML, htmlAttributes, children);
    ...
    createClass(HtmlProvider, [{
        ...
        value: function render$$1() {
            ...
            return React.createElement(
                ...
                HeadPortal && React.createElement(
                    HeadContainer,
                    ...
                    function (headComponents) {
                        return React.createElement(
                            HeadPortal,
                            null,
                            React.createElement(HeadFragment$$1, headComponents)
                        );
                    }
                )
            );
...
function withHtml(renderHtml$$1) {
    ...
    var getHtmlState = function (props, options) {
        return mapHtmlToState(renderHtml$$1(props), _extends({}, options, { permitAppChildren: false }));
    };

    return function (Component$$1) {
        var componentName = componentDisplayName || getDisplayName(Component$$1);


        var HtmlHOC = createHtmlHOC(getHtmlState, Component$$1 && Component$$1.defaultProps);
        ...
        var HtmlMetadataHOC = function (_ref) {
            ...
            var wrappedChildren = Component$$1 === null ? null : typeof children !== "undefined" ? React.createElement(
                Component$$1,
                props,
                children
        ...
        HtmlMetadataHOC.WrappedComponent = Component$$1;
        return hoistNonReactStatic(HtmlMetadataHOC, Component$$1);
    };
}
...
function hydrateHtml$1(element) {
    var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    ...
    return prepareForRender(htmlMetadata, element, container, function (rootElement, appContainer) {
        ...
        // 1. hydrate the app
        // 2. hydrate the <head>
        hydrateHtml(rootElement, appContainer, function () {
            ...
            hydrateHtml(createHeadChildren(rootElement, htmlMetadata), headDomNode, function () {
...
function renderHtml$1(element) {
    ...
    return prepareForRender(htmlMetadata, element, container, function (rootElement, appContainer) {
        renderHtml(rootElement, appContainer, callback);
    });
}
...
function prepareForRender(htmlMetadata, element, container, renderCallback) {
    ...
    var appContainer = container || document.querySelector(getAppContainerProps(isValidElement(element) ? element : rootElement).querySelector);
...
export default Html;
export { Html, withHtml, renderHtml$1 as renderHtml, hydrateHtml$1 as hydrateHtml, HtmlProvider, HtmlMetadata, withHtmlProvider, createHeadChildren };
